#!/usr/bin/env python3
"""5-minute candle strategy: EMA 10/20/30 crossover + RSI55 -> ATM option entry."""
...
LOG_PATH = STRATEGIES_DIR / "logs" / "agent5min.log"
...
DATABASE_PATH = STRATEGIES_DIR / "databases" / "NiftyFNOTop100_5min.db"
ENTERED_PATH = STRATEGIES_DIR / "data" / "entered_5min.json"
STRATEGY_NAME = "EMA_10_20_30_CROSSOVER_5MIN"

# --- Tuning constants --------------------------------------------------------
LIVE = True                          # True = send real orders
SKIP_MARKET_HOURS = True             # True = run outside market hours (test mode)
FORCE_ENTRY = True                   # True = place test order ignoring signal (test mode)
POLL_SECONDS = 30                    # test: 30s (prod: 5 * 60)
# POLL_SECONDS = 30                   #   <-- test: 30s
# POLL_SECONDS = 5 * 60              #   <-- prod: 5 minutes
...
def ema_rsi_entry_signal(candles: list[Candle]) -> tuple[bool, dict]:
    """Detect multi-EMA crossover + RSI entry on 5-min candles.

    Entry rules:
      - Previous candle: close < EMA10 AND EMA10 < EMA20 AND EMA20 < EMA30
      - Current candle: close > EMA10
      - EMA10 crosses above EMA20:  prev EMA10 <= EMA20 AND curr EMA10 > EMA20
      - EMA20 crosses above EMA30:  prev EMA20 <= EMA30 AND curr EMA20 > EMA30
    ...
    ema30_vals = ema(closes, 30)
    rsi_vals = rsi(closes)

    prev_idx, curr_idx = -2, -1
    required = (ema10_vals[prev_idx], ema20_vals[prev_idx], ema30_vals[prev_idx], rsi_vals[prev_idx],
                ema10_vals[curr_idx], ema20_vals[curr_idx], ema30_vals[curr_idx], rsi_vals[curr_idx])
    ...
    prev_ema20, curr_ema20 = ema20_vals[prev_idx], ema20_vals[curr_idx]
    prev_ema30, curr_ema30 = ema30_vals[prev_idx], ema30_vals[curr_idx]
    ...
    details = {
        ...
        "prev_ema20": prev_ema20, "curr_ema20": curr_ema20,
        "prev_ema30": prev_ema30, "curr_ema30": curr_ema30,
        "prev_rsi": prev_rsi, "curr_rsi": curr_rsi,
    }

    cond_prev_below = (prev_close < prev_ema10 and
                       prev_ema10 < prev_ema20 and
                       prev_ema20 < prev_ema30)
    cond_curr_cross_above = curr_close > curr_ema10
    cond_ema10_above_ema20 = prev_ema10 <= prev_ema20 and curr_ema10 > curr_ema20
    cond_ema20_above_ema30 = prev_ema20 <= prev_ema30 and curr_ema20 > curr_ema30
    cond_rsi_cross = prev_rsi <= RSI_ENTRY_THRESHOLD and curr_rsi > RSI_ENTRY_THRESHOLD

    triggered = (cond_prev_below and cond_curr_cross_above and
                 cond_ema10_above_ema20 and cond_ema20_above_ema30 and cond_rsi_cross)
    if triggered:
        log_message(f"ema_rsi_entry: PREV close={prev_close} < ema10={prev_ema10} < "
                    f"ema20={prev_ema20} < ema30={prev_ema30} -> "
                    f"CURR close={curr_close} > ema10={curr_ema10} | "
                    f"ema10>ema20 | ema20>ema30 | rsi {prev_rsi:.1f}->{curr_rsi:.1f}")
...
def log_signal(symbol: str, signal_type: str, details: dict) -> None:
    log_message(f"SIGNAL_EVAL: {symbol} type={signal_type} "
                ...
                f"prev_ema20={details.get('prev_ema20')} curr_ema20={details.get('curr_ema20')} "
                f"prev_ema30={details.get('prev_ema30')} curr_ema30={details.get('curr_ema30')} "
...
def enter_position_if_triggered(client: FyersClient, symbol: str,
    ...
    """
    SCRIPT_NAME = "BuyCallOption102030EmaCrossover5min.py"
    ...
    for pos in positions:
        ...
        if pos_symbol == option_symbol and net_qty != 0:
            log_message(f"ENTRY_SKIP: {symbol} already has open option position ({option_symbol}, net_qty={net_qty})")
    ...
    meta = {"strategy": STRATEGY_NAME, "signal": "ENTRY", "description": f"EMA 10/20/30 crossover entry for {symbol} at LTP {ltp}"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="5-min EMA/RSI options strategy")
